...
app.post('/get-user-training-data', async (req, res) => {
    const { email } = req.body;
    try {
        // Получаем дни недели пользователя
        const weekDaysResult = await pool.query(`SELECT * FROM training_days_weekly WHERE email = $1`, [email]);
        const weekDays = weekDaysResult.rows;

        // Получаем общее количество сессий и часов
        const totalSessionsHoursResult = await pool.query(`SELECT sessions, hours FROM total_trainings WHERE email = $1`, [email]);
        const totalSessionsHours = totalSessionsHoursResult.rows[0] || { sessions: 0, hours: 0 }; // Устанавливаем значения по умолчанию, если данные отсутствуют

        // Получаем информацию о стриках
        const streakResult = await pool.query(`SELECT streak FROM users_streaks WHERE email = $1`, [email]);
        const streak = streakResult.rows[0] ? streakResult.rows[0].streak : 0; // Устанавливаем значение по умолчанию для стриков

        // Проверяем, есть ли данные о неделях
...
app.post('/mark-training-attended', async (req, res) => {
    const { email, weekDay } = req.body;

    try {
        await pool.query(`UPDATE training_days_weekly SET attended = TRUE WHERE email = $1 AND week_day = $2;`, [email, weekDay]);
...
app.post('/set-training-plan', async (req, res) => {
    ...
    try {
        ...
        const passedDays = daysArray.filter(day => {
            ...
            return targetDayIndex < dayIndex; // Сравниваем индексы дней
        });

        for (const day of daysArray) {
            const attended = false; // Если день прошел, то attended = false
            const toShow = !passedDays.includes(day); // Если день еще не прошел, то toShow = true

            await pool.query(
                `INSERT INTO training_days_weekly (email, week_day, attended, to_show) 
         VALUES ($1, $2, $3, $4)`,
